...
== WsClient Design
Straight forward. Create basic HTTP client. Request websocket connection.
[source,java]
-----------------
...
Vertx it is not intended to execute blocking code. It always expects an event as input. Since the Vertx Periodic task can only be scheduled in milliseconds magnitudes and as we want to be in nanoseconds magnitudes, we have to make a workaround.
Executing an infinite loop in Vertx require us to encapsulate the code in a special manner. Notice that this is intended to be a massive load generator. As we already know that the server will pause the socket for backpressure, from the client side prespective the socket writing queue will also get eventually full. On a real case scenario a circuit-breaker will be recommended.
[source,java]
-----------------
